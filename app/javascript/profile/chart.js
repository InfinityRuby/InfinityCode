...
document.addEventListener('turbolinks:load', () => {
  fetch(`/api/v1/users/1/completed_rate`)
    ...
    .then(percent => {
      ...
      document.querySelector(".amount").textContent = easy + medium + hard 
      document.querySelector(".easy").lastElementChild.innerHTML = `<span>${easy}/${totalEasy}</span>`
      document.querySelector(".medium").lastElementChild.innerHTML = `<span>${medium}/${totalMedium}</span>`
      document.querySelector(".hard").lastElementChild.innerHTML = `<span>${hard}/${totalHard}</span>`
